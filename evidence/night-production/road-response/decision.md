...
## Exact control restored and verified

Candidate source remains published at `a5debfb`. Restore only its three runtime/scene changes from control `8df99ad`, and restore the retained exact control app. [Restoration identity](restoration-validation.json) and [final verification](final/verification.json) confirm GUID `40bed5f53c2449418e7fb56bf59739f6`, all 197 source and 189 app hashes, and the scene SHA. The validator's resolution guard and explicit reference support remain.

Fresh existing tests pass 42/42. The separate real-time native race exits zero after three laps in128.32s, zero recoveries for all six racers in the final telemetry, two successful restart launches and three successful countdown-pause checks. Six PNGs pass CRC/decompression at1920×1080. Parent inspects the results screenshot. The 60-second sample measures6,967 intervals on M2Max at1080p/VSync1: mean8.61ms, P959.25ms, P9916.74ms, memory243.3MiB. Its higher P99 than the older baseline is retained; it is not a performance measurement of the rejected candidate. No build, bake or encoder overlaps the sample.

The [technique feasibility report](../../../docs/road-reviews/005-reflection-feasibility.md) recommends one isolated native preview-SSR prototype on the current package, with a compile/type/resource/pass gate before evaluating any visual effect. No new reflection technique is implemented here. The one-smoothness experiment is complete; the production road-response target, continuous watched-motion and manual/aspect acceptance remain open.
