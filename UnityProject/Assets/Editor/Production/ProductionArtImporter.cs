...
namespace VectorRush.Editor
{
    ...
    public static class ProductionArtImporter
    {
            ...
            string worldDestinationRoot = "Assets/World/NocturneProduction")
        {
            ...
            VerifyGeneratedAssets(package, worldFinal);
            return new ProductionImportResult
            {
                revision = package.manifest.revision,
                artAssetRoot = artFinal,
        ...
        public static ProductionImportResult ResolveExistingPayload(
            string absolutePackageDirectory,
            string expectedCourseHash,
            string directAssetRoot = "Assets/Resources/ExperienceArt",
            string worldDestinationRoot = "Assets/World/Experience")
        {
            ValidatedProductionPackage package = ValidatePackage(absolutePackageDirectory, expectedCourseHash);
            string artRoot = ValidateAssetRoot(directAssetRoot, nameof(directAssetRoot));
            string worldRoot = ValidateAssetRoot(worldDestinationRoot, nameof(worldDestinationRoot));
            if (!AssetDatabase.IsValidFolder(artRoot)) throw new DirectoryNotFoundException("Direct Unity payload is missing: " + artRoot);
            VerifyExistingPayload(package, artRoot);
            ConfigurePayloadImporters(package, artRoot);
            ValidateImportedBounds(package, artRoot);

            string worldFinal = worldRoot + "/" + package.manifest.revision;
            if (AssetDatabase.IsValidFolder(worldFinal))
            {
                VerifyGeneratedAssets(package, worldFinal);
            }
            else
            {
                EnsureAssetFolder(worldRoot);
                string worldStage = worldRoot + "/ImportStaging-" + Guid.NewGuid().ToString("N");
                try
                {
                    EnsureAssetFolder(worldStage);
                    CreatePersistentMaterialsAndPrefabs(package, artRoot, worldStage, true);
                    AssetDatabase.SaveAssets();
                    MoveAssetOrThrow(worldStage, worldFinal);
                    AssetDatabase.SaveAssets();
                }
            ...
            {
                revision = package.manifest.revision,
                artAssetRoot = artRoot,
                payloadAssetRoot = string.Empty,
                importedAssetRoot = artRoot,
                worldAssetRoot = worldFinal,
                package = package
            };
        }

        static void VerifyExistingPayload(ValidatedProductionPackage package, string directAssetRoot)
        {
            var required = new HashSet<string>(StringComparer.Ordinal) { "manifest.json", "layout.json", "asset-stats.json" };
            foreach (ProductionAssetRecord asset in package.manifest.assets)
            {
                required.Add(Normalize(asset.lod0));
                required.Add(Normalize(asset.lod1));
            }
            foreach (ProductionMaterialRecord material in package.manifest.materials)
            {
                required.Add(Normalize(material.baseColor));
                required.Add(Normalize(material.normal));
                required.Add(Normalize(material.metallicSmoothness));
                required.Add(Normalize(material.occlusion));
                if (!string.IsNullOrWhiteSpace(material.emission)) required.Add(Normalize(material.emission));
            }
            foreach (string relative in required)
            {
                string source = Path.Combine(package.root, relative.Replace('/', Path.DirectorySeparatorChar));
                string direct = AssetPathToAbsolute(directAssetRoot + "/" + relative);
                if (!File.Exists(direct)) throw new InvalidDataException("Direct Unity payload is missing package file: " + relative);
                if (!string.Equals(Hash(File.ReadAllBytes(source)), Hash(File.ReadAllBytes(direct)), StringComparison.OrdinalIgnoreCase))
                    throw new InvalidDataException("Direct Unity payload differs from immutable package: " + relative);
            }
        }

        static void VerifyGeneratedAssets(ValidatedProductionPackage package, string worldFinal)
        {
            foreach (ProductionAssetRecord asset in package.manifest.assets)
        ...
        static void ValidateImportedBounds(ValidatedProductionPackage package, string artStage)
        {
            foreach (ProductionAssetRecord asset in package.manifest.assets)
            {
                ValidateImportedBounds(asset, artStage + "/" + Normalize(asset.lod0), "LOD0", .005f);
                // LOD1 may remove small roofline details while preserving the authored mass.
                // Keep this tolerance tight enough that axis flips and missing major forms fail.
                ValidateImportedBounds(asset, artStage + "/" + Normalize(asset.lod1), "LOD1", .015f);
            }
        }

        static void ValidateImportedBounds(ProductionAssetRecord asset, string modelPath, string lod, float relativeTolerance)
        {
            ...
            {
                float tolerance = Mathf.Max(.02f, Mathf.Abs(expectedSize[axis]) * relativeTolerance);
        ...
        static void CreatePersistentMaterialsAndPrefabs(ValidatedProductionPackage package, string artStage, string worldStage, bool experienceLodPolicy = false)
        {
            ...
            foreach (ProductionMaterialRecord record in package.manifest.materials)
            {
                ...
                if (record.uvMode == "metric" && record.metersPerTile != null && record.metersPerTile.Length == 2)
                    tiling = new Vector2(1f / record.metersPerTile[0], 1f / record.metersPerTile[1]);
            ...
            foreach (ProductionAssetRecord asset in package.manifest.assets)
                CreatePrefab(asset, artStage, worldStage, materials, experienceLodPolicy);
        }

        static void CreatePrefab(ProductionAssetRecord asset, string artStage, string worldStage, Dictionary<string, Material> materials, bool experienceLodPolicy)
        {
            ...
            try
            {
                ...
                bool closeModule = asset.id.IndexOf("podium", StringComparison.OrdinalIgnoreCase) >= 0 ||
                                   asset.id.IndexOf("guidance", StringComparison.OrdinalIgnoreCase) >= 0;
                float lod0Height = experienceLodPolicy ? closeModule ? .35f : .20f : .55f;
                float lod1Height = experienceLodPolicy ? closeModule ? .06f : .035f : .12f;
                var lodLevel0 = new LOD(lod0Height, lod0Renderers) { fadeTransitionWidth = experienceLodPolicy ? .15f : 0f };
                var lodLevel1 = new LOD(lod1Height, lod1Renderers) { fadeTransitionWidth = experienceLodPolicy ? .15f : 0f };
                group.SetLODs(new[] { lodLevel0, lodLevel1 });
                group.fadeMode = LODFadeMode.CrossFade;
                group.animateCrossFading = experienceLodPolicy;
