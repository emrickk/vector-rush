...
half3 BillboardPlayback(float2 uv,int campaign,float phase,float aspect,float style,float2 crop,float minimumMip)
{
    ...
    if(style>.5 && style<1.5){
        float2 q=float2(frac(uv.x*.8+t*.035),(campaign+lerp(.08,.92,uv.y))/4);
    ...
    float frame=campaign==2 ? (1-abs(frac(t/10)*2-1))*63 : frac(t/5)*64;
    ...
    if(style<.5 && aspect>1.05 && uv.x>.52)
    {
        ...
        float wave=.18+.10*sin(card.x*19+t*3)+.05*sin(card.x*47-t*4);
    ...
    if(style<.5 && uv.y<.105)
    {
        float2 q=float2(frac(uv.x*aspect*.55+t*.035),(campaign+uv.y/.105)/4);
