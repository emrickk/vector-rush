Shader "VectorRush/City Glass Reflection"
{
    Properties
    {
        ...
        Pass
        {
            ...
            half3 ReflectSigns(float3 position,float3 direction,half roughness)
            {
                ...
                [loop] for(int n=0;n<(int)_SignCount;n++)
                {
                    ...
                    float3 normal=LOAD_TEXTURE2D(_SignData,int2(3,n)).xyz;
                    ...
                    result+=SAMPLE_TEXTURE2D_ARRAY_LOD(_SignImages,sampler_SignImages,uv,origin.w,2+roughness*3).rgb*edge*4;
                }
                return result;
            }
