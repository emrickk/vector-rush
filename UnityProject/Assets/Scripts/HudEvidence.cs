...
namespace VectorRush
{
    ...
    public sealed class HudEvidence : MonoBehaviour
    {
        ...
        IEnumerator Run()
        {
            ...
            try
            {
                ...
                if(report.error==null)
                {
                    ...
                    {
                        yield return CaptureGroup("04-boost",RacePhase.Racing,true);
                        if(Array.IndexOf(Environment.GetCommandLineArgs(),"-hudMotionEvidence")>=0) yield return CaptureBoostMotion();
                    }
                }
                if(report.error==null)
                {
        ...
        IEnumerator CaptureBoostMotion()
        {
            var prefs=PlayerPreferences.Current;
            bool originalReduced=prefs.ReducedInterfaceMotion;
            int originalRate=Time.captureFramerate;
            var hud=FindAnyObjectByType<RaceHUD>();
            try
            {
                prefs.SetReducedInterfaceMotion(false);
                Time.captureFramerate=60;
                Screen.SetResolution(1920,1080,FullScreenMode.Windowed);
                float deadline=Time.realtimeSinceStartup+30;
                while((!player.IsBoosting || player.SpeedKph<210 || Screen.width!=1920 || Screen.height!=1080) && Time.realtimeSinceStartup<deadline) yield return null;
                if(!player.IsBoosting || player.SpeedKph<210) { Fail("No high-speed boost for HUD motion capture."); yield break; }
                string motionFolder=Path.Combine(folder,"motion");
                Directory.CreateDirectory(motionFolder);
                for(int i=0;i<120;i++)
                {
                    // Last half-second proves suppression during the same real run.
                    bool reduced=i>=90;
                    prefs.SetReducedInterfaceMotion(reduced);
                    yield return null;
                    yield return new WaitForEndOfFrame();
                    string file="motion/frame-"+i.ToString("D4")+".png";
                    ScreenCapture.CaptureScreenshot(Path.Combine(folder,file));
                    report.motion.Add(new MotionView{file=file,raceTime=director.RaceTime,speed=player.SpeedKph,boosting=player.IsBoosting,reduced=reduced,amount=hud.ArcadeBoostAmount});
                }
            }
            finally { prefs.SetReducedInterfaceMotion(originalReduced); Time.captureFramerate=originalRate; }
        }
        ...
        {
            public string scope,startedUtc,finishedUtc,unityVersion,error; public string buildGuid=Application.buildGUID; public bool complete,finalLapTransitionObserved;
            public float finishedRaceTime,finalLapObservationRaceTime; public int completedLaps,playerRecoveries;
            public List<View> views=new List<View>();
            public string motionScope="Optional native 120-frame sequence at 60 simulation fps. Frames 90–119 enable Reduced Interface Motion in memory, restored afterward. Normal physics/autopilot; no speed or boost injection. Silent display evidence, not real-time performance.";
            public List<MotionView> motion=new List<MotionView>();
        }
        [Serializable] sealed class MotionView { public string file; public float raceTime,speed,amount; public bool boosting,reduced; }
