...
namespace VectorRush
{
    public sealed class WorldBuilder : MonoBehaviour
    {
        ...
        {
            track=path;track.Ensure();OpeningCity.Prepare(track);
            ...
            gameObject.AddComponent<NightDistrict>().Build(this,track);gameObject.AddComponent<NightTrackLighting>().Build(this,track);
            if(OpeningCity.Enabled)gameObject.AddComponent<OpeningCity>().Build(this,track);
            BuildLighting();
        }
        ...
        {
            float t=track.ClosestProgress(f.Position);
            if(OpeningCity.Enabled&&t>=.024f&&t<=.326f)return;
