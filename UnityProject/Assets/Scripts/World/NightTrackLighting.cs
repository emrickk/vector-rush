...
namespace VectorRush
{
    ...
    {
        public const string WarmPrimaryPortal = "Warm gallery primary portal structure";
        public const string WarmContinuousShell = "Warm gallery continuous shell";
        public const string WarmCassettes = "Warm gallery recessed ceramic cassettes";
        public const string WarmReturns = "Warm gallery folded service returns";
        public const string WarmHousings = "Warm gallery light and maintenance housings";
        public const string WarmDiffusers = "Warm gallery integrated diffusers";
        public const string WarmMarkings = "Warm gallery maintenance markings";
        public const string WarmSurfaceWash = "Warm gallery concealed surface wash";
        public const string WarmRoadPool = "Warm gallery road pool";
        public const int WarmSurfaceWashCount = 12;
        public const int WarmRoadPoolCount = 6;
        public static readonly string[] ReplaceableWarmRendererNames = {
            WarmPrimaryPortal, WarmCassettes, WarmReturns, WarmHousings, WarmDiffusers, WarmMarkings
        };
        ...
        public void Build(WorldBuilder world,TrackPath track)
        {
            ...
            for(int i=0;i<58;i++){
                ...
                if(OpeningRoadLightResponse.Enabled&&OpeningRoadLightResponse.ContainsFixture(i))OpeningRoadFixture(light,f,side,i,warm);
                else if(OpeningFinishPreview.SurfaceEnabled&&i>=7&&i<=18)OpeningFixture(light,f,side,i);
                }
        ...
        static void OpeningRoadFixture(Light light,TrackFrame frame,float side,int index,bool warm)
        {
            // Re-aim the existing physical track fixture across the deck. Alternating sides and
            // varying reach produce moving, bank-following pools without adding lights or painting
            // color into road coordinates. The two wrap fixtures cover the launch before progress 0.
            int phase=(index+2)%5;
            float forward=phase==0?10f:phase==1?6.5f:phase==2?12f:phase==3?8f:5f;
            float across=phase%2==0?4.8f:2.2f;
            light.transform.position=frame.Position+frame.Right*side*9.5f+frame.Up*12.48f;
            Vector3 target=frame.Position+frame.Forward*forward-frame.Right*side*across;
            light.transform.rotation=Quaternion.LookRotation((target-light.transform.position).normalized,frame.Forward);
            light.color=warm?new Color(1f,.31f,.07f):new Color(.16f,.72f,1f);
            // Unity's physical falloff at this 12.5 m mounting height made the first 1180/1320 cd
            // trial nearly invisible in whole-frame review. This remains below the source count of
            // the control and changes illuminance only, with dark gaps retained by the narrow cone.
            light.intensity=(warm?3000f:3600f)*(phase==1?.72f:phase==4?.82f:1f);
            light.range=50f;
            // A broad outer penumbra avoids a visible cone boundary as the bank rotates beneath
            // the camera while the narrower inner cone keeps the highlight source legible.
            light.spotAngle=82f;
            light.innerSpotAngle=24f;
            light.gameObject.name+=" / opening road response";
        }
            ...
            Material ceramic,Material trim,Material housing,Material diffuser,Material paint)
        {
            ...
            for(int i=0;i<7;i++){
                ...
                for(int side=-1;side<=1;side+=2){
                    ...
                    var wash=new GameObject(warm?WarmSurfaceWash:"Cool gallery concealed surface wash");
                ...
                var road=new GameObject(warm?WarmRoadPool:"Cool gallery road pool");road.transform.SetParent(transform,false);
            ...
            if(warm){
                Combine(WarmPrimaryPortal,ribs,structure);Combine(WarmContinuousShell,shells,ceiling);
                Combine(WarmCassettes,panels,ceramic);Combine(WarmReturns,metal,trim);
                Combine(WarmHousings,dark,housing);Combine(WarmDiffusers,lamps,diffuser);
                Combine(WarmMarkings,labels,paint);
            }else{
                const string prefix="Cool gallery ";
