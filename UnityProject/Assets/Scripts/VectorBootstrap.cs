    ...
    public sealed class VectorBootstrap : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            if (!production)
            {
                ...
                OpeningRoadLightResponse.LogConfiguration();
            }
            Instance=this;
            ...
            else
            {
                ...
                AAABaselineIntegration.TryAttach(world,Track);
            ...
            chase.MotionPresentationEnabled = production && productionWorld.artRevision.StartsWith("stage1-") && Array.IndexOf(Environment.GetCommandLineArgs(), "-motionBaseline") < 0;
            Director=new GameObject("Race director").AddComponent<RaceDirector>();Director.Initialize(Track,racers,production?productionWorld.courseHash:null,new RaceRecords(Stage2EvidenceProfile.Directory));
            new GameObject("Replay presentation").AddComponent<ReplayPolishController>().Initialize(Director,Stage2EvidenceProfile.Directory);
            ...
            AttachIntegratedPresentation(racers[0], audio);
            gameObject.AddComponent<RaceEvidence>();
            if(production) { if(!HudEvidence.TryStart(this)) ProductionEvidence.TryStart(this); }
            else if(!PaceEvidence.TryStart(this) && !EnvironmentEvidence.TryStart(this) && !RoadSurfaceEvidence.TryStart(this) && !HudEvidence.TryStart(this) && !ThrottleEvidence.TryStart(this)) ShipInspection.TryStart(this);
        }
        ...
        void AttachIntegratedPresentation(HoverVehicle player, RaceAudio audio)
        {
            if (!player || !audio) return;
            VehicleVFX effects = player.GetComponent<VehicleVFX>();
            if (effects)
            {
                var bridge = gameObject.AddComponent<VehicleContactAudioBridge>();
                bridge.Initialize(effects, audio, Director);
            }

            MotionBlur blur = null;
            foreach (Volume volume in FindObjectsByType<Volume>(FindObjectsInactive.Exclude, FindObjectsSortMode.None))
            {
                if (volume.gameObject.scene != gameObject.scene || !volume.isGlobal) continue;
                VolumeProfile runtimeProfile = volume.profile;
                if (runtimeProfile && runtimeProfile.TryGet(out blur)) break;
            }
            if (blur == null) return;
