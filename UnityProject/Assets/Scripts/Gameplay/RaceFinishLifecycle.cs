...
namespace VectorRush
{
    ...
    public sealed class RaceFinishLifecycle
    {
        ...
        public float FinishDeadline => PlayerFinished ? playerFinishTime + timeoutSeconds : float.PositiveInfinity;
        ...
        {
            BeginStep(deltaSeconds, paused);
            FinalizeTimeouts();
        }

        /// <summary>Advance the clock without resolving timeouts so crossings in this step can be adjudicated first.</summary>
        public void BeginStep(float deltaSeconds, bool paused = false)
        {
        ...
        public RacerFinishRecord RecordFinish(string racerId, float crossingTime)
        {
            ...
            if (!identity.IsPlayer && PlayerFinished && crossingTime > FinishDeadline) return null;
        ...
        {
            if (!PlayerFinished || SimulationTime < FinishDeadline) return;
            foreach (RacerIdentity identity in racers)
            {
