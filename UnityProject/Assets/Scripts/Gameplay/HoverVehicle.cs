...
namespace VectorRush
{
    ...
    public sealed class HoverVehicle : MonoBehaviour
    {
        ...
        public bool BoostRequiresRelease => boostExhausted;
        ...
        bool diagnosticStartOverride;
        float lostTime, stoppedTime, visualBank;
        float aiLane, nextLaneDecision, diagnosticStartDistance, diagnosticStartLane;
        ...
        public void ResetForRace()
        {
            ...
            float distanceBehind = diagnosticStartOverride ? diagnosticStartDistance : StartingDistanceBehindLine(IsPlayer, gridIndex);
            float progress = Mathf.Repeat(-distanceBehind / track.Length, 1f);
            ...
            Reposition(progress, diagnosticStartOverride ? diagnosticStartLane : gridIndex % 2 == 0 ? -4f : 4f);
        }

        internal void SetDiagnosticStartingPosition(float distanceBehindLine, float lateral)
        {
            diagnosticStartOverride = true;
            diagnosticStartDistance = Mathf.Max(1f, distanceBehindLine);
            diagnosticStartLane = Mathf.Clamp(lateral, -Mathf.Min(5.8f, track.Width * .265f), Mathf.Min(5.8f, track.Width * .265f));
        }

        public static float StartingDistanceBehindLine(bool isPlayer, int index)
        {
            if (isPlayer) return 28f;
            if (index <= 2) return 14f;
            if (index == 3) return 24f;
            return 40f + Mathf.Max(0, index - 5) * 12f;
        }
        ...
        void DriveAI(TrackFrame frame)
        {
            ...
            {
                if (other == this || !CanInfluenceTraffic(other)) continue;
            ...
            if(aiEdgeGuard)
            {
                ...
                foreach(var other in racers)
                {
                    if(other==this || !CanInfluenceTraffic(other) || Mathf.Abs(SignedTrackGap(other))>12f) continue;
        ...
        public static bool CanInfluenceTraffic(HoverVehicle racer)
        {
            // Resolved racers are made kinematic. They remain in the race list for
            // ordered results, but must not act like stopped traffic at the finish.
            return racer && racer.Body && !racer.Body.isKinematic;
        }
        ...
        float SignedTrackGap(HoverVehicle other) => (other.RaceProgress - RaceProgress) * track.Length;
        ...
        void Recover(string reason)
        {
            ...
            RaceDirector.Instance?.NotifyRecovery(this);
            float preferredLane = IsPlayer && !AutopilotForTesting ? 0f : aiLane;
            var occupiedLanes = new List<float>();
            var director = RaceDirector.Instance;
            float targetRaceDistance = ProgressTracker.CompletedLaps + progress;
            if (director != null)
            {
                foreach (HoverVehicle other in director.Racers)
                {
                    if (other == this || !CanInfluenceTraffic(other)) continue;
                    if (Mathf.Abs((other.RaceProgress - targetRaceDistance) * track.Length) > 18f) continue;
                    occupiedLanes.Add(OtherLane(other));
                }
            }
            Reposition(progress, SelectRecoveryLane(track.Width, preferredLane, occupiedLanes));
            ProgressTracker.NotifyRespawn(progress);
            lostTime = stoppedTime = 0f;
            Boost01 = Mathf.Max(0f, Boost01 - .15f);
        }

        public static float SelectRecoveryLane(float width, float preferredLane, IReadOnlyList<float> occupiedLanes)
        {
            float spacing = Mathf.Min(5.8f, Mathf.Max(0f, width) * .265f);
            float[] candidates = { Mathf.Clamp(preferredLane, -spacing, spacing), 0f, -spacing, spacing };
            float bestLane = candidates[0];
            float bestClearance = -1f;
            for (int i = 0; i < candidates.Length; i++)
            {
                bool duplicate = false;
                for (int previous = 0; previous < i; previous++)
                    if (Mathf.Abs(candidates[previous] - candidates[i]) < .01f) { duplicate = true; break; }
                if (duplicate) continue;
                float clearance = float.PositiveInfinity;
                if (occupiedLanes != null)
                    for (int occupied = 0; occupied < occupiedLanes.Count; occupied++)
                        clearance = Mathf.Min(clearance, Mathf.Abs(candidates[i] - occupiedLanes[occupied]));
                if (clearance > bestClearance)
                {
                    bestClearance = clearance;
                    bestLane = candidates[i];
                }
            }
            return bestLane;
        }
