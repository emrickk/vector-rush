...
namespace VectorRush
{
    ...
    public sealed class EnvironmentEvidence : MonoBehaviour
    {
        ...
        int originalCaptureRate, simulationCaptureRate = 24;
        float originalTimeScale;
        bool originalAutopilot, initialized;
        bool fullLap,stillsOnly,openingStills,gallery;
        ...
        public static bool TryStart(VectorBootstrap bootstrap)
        {
            ...
            evidence.gallery=Array.IndexOf(args,"-environmentGallery")>=0;
            if(evidence.gallery){
                evidence.fullLap=false;evidence.stillsOnly=false;
                evidence.anchors=new[]{.83f,.85f,.86f,.868f,.878f,.888f,.898f,.91f,.93f};
                evidence.labels=new[]{"01-approach","02-threshold","03-entry","04-early-interior","05-middle","06-late-interior","07-exit","08-release","09-city"};
            }
        ...
        IEnumerator Run()
        {
            ...
            report.exemplarMode=ExemplarMode();report.exemplarAttached=FindFirstObjectByType<AAABaselineIntegration>()!=null;
            var pipeline=GraphicsSettings.currentRenderPipeline as UniversalRenderPipelineAsset;
            report.additionalLightShadowsSupported=pipeline&&pipeline.supportsAdditionalLightShadows;
            ...
            int frameCount=gallery?288:(fullLap?1440:360);
            report.simulationCaptureRate=simulationCaptureRate;
            report.scope = "Actual native racing camera and normal hover physics. Existing test autopilot drives; no camera, pose, lap, energy, speed or race phase is injected. Time.captureFramerate=" + simulationCaptureRate + " is set before StartRace for repeatable simulation. The " + frameCount + "-frame passage is " + (frameCount/(float)simulationCaptureRate).ToString("0.##") + " simulation seconds, not a frame-time/performance measurement. Camera transforms, FOV, all racer rigidbody poses/velocities and player visual transform are recorded every frame. " + anchors.Length + " anchors are the first recorded frames crossing fixed track-progress thresholds. An optional prior report compares actual anchor poses; it never overwrites them. PNGs and metadata are rendering evidence, not independent visual/motion acceptance. The exported picture sequence contains no audio.";
            ...
            bool success = false;
            try {
                player.AutopilotForTesting = true; Time.timeScale = 1; Time.captureFramerate = simulationCaptureRate;
                // A fixed frame warm-up keeps startup deterministic even when
                // the candidate contains more renderers and lights.
                for(int frame=0;frame<simulationCaptureRate*2;frame++)yield return null;
                // Start immediately after a fixed step. The project simulates
                // physics at 100 Hz, so an arbitrary render-frame start can
                // otherwise offset two runs by one 10 ms physics step.
        ...
        static string ExemplarMode()
        {
            string[] args=Environment.GetCommandLineArgs();int index=Array.IndexOf(args,AAABaselineIntegration.Flag);
            return index>=0&&index+1<args.Length?args[index+1]:"off";
        }
        [Serializable] public sealed class Report {
            public string scope, startedUtc, finishedUtc, reference,unityVersion,buildGuid,gpu;
            public int sceneRenderers,activeLights,simulationCaptureRate;
            public string exemplarMode;
            public bool complete, matchedWithinTolerance,exemplarAttached,additionalLightShadowsSupported;
