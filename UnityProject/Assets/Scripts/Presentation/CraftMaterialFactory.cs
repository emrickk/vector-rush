...
namespace VectorRush
{
    public sealed class CraftMaterialFactory : IDisposable
    {
        ...
        public Material CreateMapped(string name, string key, Color tint, float smoothness, float metallic)
        {
            ...
            if (library.stage1Finish)
            {
                string stage = "ExperienceArt/ship/vrx_ship_" + key.ToLowerInvariant() + "_a_";
                color = Resources.Load<Texture2D>(stage + "base") ?? color;
                normal = Resources.Load<Texture2D>(stage + "normal") ?? normal;
                mask = Resources.Load<Texture2D>(stage + "ms") ?? mask;
                occlusion = Resources.Load<Texture2D>(stage + "ao") ?? occlusion;
            }
            ...
            if (library.stage1Finish)
            {
                var energy = Resources.Load<Texture2D>("Stage1/ShipEnergy/Stage1Energy_" + key);
                if (energy)
                {
                    material.SetTexture("_EmissionMap", energy);
