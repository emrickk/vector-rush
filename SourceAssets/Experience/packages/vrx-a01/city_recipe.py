...
def bridge():
    box("Deep connector tray",(0,1.1,0),(8,2.2,28),"slate",bevel=.1)
    hall("Occupied connector",0,2.2,0,7,5,27)
    box("Connector roof",(0,7.8,0),(8.6,.9,29),"mineral")
    for s in [-1,1]:
        for z in [-11,-4,3,10]:
            beam("Exposed bridge diagonal",(s*4,2,z),(s*4,7,z+4),.23)
def plant():
...
for k in range(60 if EXPANDED else 24):
    ...
    ident=towers[(k*3+k//5)%len(towers)]
...
if EXPANDED:
    for ident,ps,lateral,vertical in [
        ...
        ("vrx_city_bridge_a",[.095,.255,.46],52,12),
        ("vrx_city_plant_a",[.037,.13,.27,.43,.61],33,-2),
...
target=Vector((110,75 if EXPANDED else 10,35))
cam.location=target+Vector((270,-400,245));cam.rotation_euler=(target-cam.location).to_track_quat("-Z","Y").to_euler()
data.type="ORTHO";data.ortho_scale=405 if EXPANDED else 350;scene.camera=cam
