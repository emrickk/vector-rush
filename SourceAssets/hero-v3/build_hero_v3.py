...
NAC_PROFILE=[(-.72,.80),(-.45,.965),(0,1),(.45,.965),(.72,.80),(.91,.48),(1,.06),(.93,-.47),(.64,-.87),(.28,-1),(-.28,-1),(-.64,-.87),(-.93,-.47),(-1,.06),(-.91,.48)]
CANOPY_PROFILE=[(-.76,.48),(-.45,.9),(0,1),(.45,.9),(.76,.48),(1,-.25),(.86,-.8),(-.86,-.8),(-1,-.25)]

def refined_sections(sections,steps=4):
 result=[]
 for i in range(len(sections)-1):
  a=sections[max(i-1,0)];b=sections[i];c=sections[i+1];d=sections[min(i+2,len(sections)-1)]
  for j in range(steps):
   t=j/steps
   result.append(tuple((b[k]+(c[k]-b[k])*t) if k==0 else .5*((2*b[k])+(-a[k]+c[k])*t+(2*a[k]-5*b[k]+4*c[k]-d[k])*t*t+(-a[k]+3*b[k]-3*c[k]+d[k])*t*t*t) for k in range(5)))
 result.append(sections[-1]);return result

def shade_shell(o):
 # Smooth shallow curvature, retain explicit shoulders and closure edges.
 for poly in o.data.polygons:poly.use_smooth=True
 bm=bmesh.new();bm.from_mesh(o.data)
 for e in bm.edges:
  if len(e.link_faces)==2 and e.calc_face_angle()>.56:e.smooth=False
...
def shell_panel(name,sections,indices,mat='Ivory',thickness=.085,bevel=.012,profile=PROFILE,smooth=False):
 ...
 o=mesh_obj(name,v,f,mat,bevel)
 return shade_shell(o) if smooth else o
...
canopy_sections=refined_sections([(-2.55,0,.48,.12,.024),(-2.10,0,.57,.29,.10),(-1.35,0,.68,.40,.15),(-.48,0,.70,.43,.16),(.18,0,.65,.37,.15),(.82,0,.46,.23,.045)])
shade_shell(loft('Canopy / narrow continuous sill',[(y,x,z-.018,w+.016,h+.009) for y,x,z,w,h in canopy_sections],'Graphite',.007,profile=CANOPY_PROFILE))
shade_shell(loft('Canopy / controlled convex near-black glazing',canopy_sections,'Glass',.004,profile=CANOPY_PROFILE))
...
for side in [-1,1]:
 ...
 for station in [-1.15,1.55]:
  ...
  loft('Chassis / formed nacelle mounting saddle',[(station-.21,side*1.065,-.025,.16,.085),(station+.04,side*1.065,.015,.185,.16),(station+.27,side*1.065,-.025,.14,.095)],'Ceramic',.012)
  loft('Chassis / seated central mounting flange',[(station-.23,side*.53,.02,.08,.07),(station,side*.53,.045,.10,.13),(station+.23,side*.53,.02,.08,.07)],'Metal',.008)
 # Compound nacelle envelope: low shaped forebody, cockpit waist and broad engine shoulder.
 sections=[(-3.34,side*1.50,-.075,.25,.17),(-3.05,side*1.52,-.005,.47,.28),(-2.52,side*1.57,.045,.69,.39),(-1.83,side*1.65,.05,.72,.45),(-.90,side*1.75,.015,.665,.465),(.05,side*1.76,.02,.75,.52),(1.17,side*1.72,.01,.885,.60),(2.32,side*1.66,-.005,.94,.655),(3.34,side*1.65,-.005,.86,.63)]
 detailed=refined_sections(sections)
 core_sections=[(y,x,z,w-.09,h-.04) for y,x,z,w,h in detailed]
 core_sections += [(3.34,side*1.65,-.005,.58,.52),(3.0,side*1.65,-.005,.53,.48),(2.36,side*1.65,-.005,.34,.33)]
 core=shade_shell(loft('Nacelle / closed structural cavity shell '+str(side),core_sections,'Graphite',.008,profile=NAC_PROFILE))
 # Large coherent shell groups, continuous across local profile changes.
 crown=[14,0,1,2,3,4,5]
 fore=shell_panel('Armor / shaped forward crown '+str(side),refined_sections(sections[:4]),crown,'Ivory',.07,.009,NAC_PROFILE,True)
 middle_stations=[(-1.818,side*1.651,.05,.72,.4502),sections[4],sections[5],(1.158,side*1.72,.01,.884,.599)]
 middle=shell_panel('Armor / waisted center crown '+str(side),refined_sections(middle_stations),crown,'Ivory',.07,.009,NAC_PROFILE,True)
 aft=shell_panel('Armor / engine shoulder crown '+str(side),refined_sections(sections[6:]),crown,'Ivory',.07,.009,NAC_PROFILE,True)
 outer=[5,6,7,8,9] if side==1 else [10,11,12,13,14]
 shell_panel('Armor / forward changing lower return '+str(side),refined_sections(sections[:4]),outer,'Ivory',.07,.009,NAC_PROFILE,True)
 shell_panel('Armor / aft changing lower return '+str(side),refined_sections([middle_stations[0]]+sections[4:]),outer,'Ivory',.07,.009,NAC_PROFILE,True)
 # Swept narrow inlet: a shaped mouth with a sloping ramp, cut through actual shell.
 inlet=[(-.26,-.91),(.13,-.91),(.31,.59),(.23,.75),(-.28,.75)]
 verts=[(side*1.75+side*x,y,z) for z in [.28,1.0] for x,y in inlet]
 faces=[tuple(reversed(range(5))),tuple(range(5,10))]+[(j,(j+1)%5,(j+1)%5+5,j+5) for j in range(5)]
 cutter=mesh_obj('TEMP swept inlet cutter',verts,faces,'Graphite',0)
 for target in [middle,core]:
  apply_mods(target);bpy.context.view_layer.objects.active=target
  m=target.modifiers.new('Actual swept inlet recess','BOOLEAN');m.operation='DIFFERENCE';m.solver='EXACT';m.object=cutter
  bpy.ops.object.modifier_apply(modifier=m.name);normalise(target)
 bpy.data.objects.remove(cutter,do_unlink=True)
 loft('Intake / sloping interior ramp '+str(side),[(-.85,side*1.72,.30,.12,.025),(-.1,side*1.77,.24,.19,.025),(.65,side*1.78,.21,.20,.025)],'Ceramic',.006)
 for j in range(2):box('Intake / recessed guide '+str(side)+' '+str(j),(side*1.77,.17+j*.27,.295),(.39,.04,.10),'Ceramic',.005,(-.35,0,0))
 inner=[10,11,12,13,14] if side==1 else [4,5,6,7,8]
 shell_panel('Nacelle / inner structural heat shield '+str(side),refined_sections([(y,x,z,w-.075,h-.11) for y,x,z,w,h in sections[3:-1]]),inner,'Ceramic',.035,.006,NAC_PROFILE,True)
...
scene=bpy.context.scene;scene.render.engine='CYCLES';scene.cycles.samples=48;scene.cycles.device='CPU';scene.cycles.use_denoising=False
...
for key in M:
 ...
 tri=o.modifiers.new('Final runtime triangulation before stable atlas','TRIANGULATE');bpy.ops.object.modifier_apply(modifier=tri.name)
...
views=[
 ...
 ('10-C-comparison-16x9',(-8.2,21.6,8.4),(0,-.3,.06),9.6,False,'PERSP')]
(EVIDENCE/'capture-contract.json').write_text(json.dumps({'revision':OUT.name,'resolution':[1600,1200],'neutral_white_studio':True,'bloom':False,'view_transform':'AgX','views':views,'perspective_focal_length_mm':45,'C_comparison_resolution':[1920,1080]},indent=2))
