...
def beam(name,a,b,width,depth,mat='Concrete'):
 ...
 direction=(Vector(b)-Vector(a)).normalized();reference=Vector((0,1,0))
 if abs(direction.dot(reference))>.99:reference=Vector((0,0,1))
 side=reference.cross(direction).normalized();depth_axis=direction.cross(side).normalized()
 ob.location=(Vector(a)+Vector(b))/2;ob.rotation_euler=Matrix((side,depth_axis,direction)).transposed().to_euler();return ob
...
for name,source in assets.items():
 if name=='Nocturne_ServiceWorkshop_A':
  for ob in source:ob.location.x+=29;ob.location.y-=35
