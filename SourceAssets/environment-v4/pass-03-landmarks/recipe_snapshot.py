...
ROOT=os.path.dirname(os.path.abspath(__file__));OUT=os.path.join(ROOT,'pass-03-landmarks');os.makedirs(OUT,exist_ok=True)
...
for index,(y,r,top) in enumerate([(-24,8.2,75.5),(0,9.8,90.5),(24,7.9,64.0)]):
 ...
 for j in range(6):
  ...
  ob=box('Drum shroud reinforcing fold',(0,0,0),(.36,1.1,5.9),'Titanium',.025);ob.location=loc;ob.rotation_euler.z=a
 lathe('Drum fan spindle',x,y,[(top-6.7,1.1),(top-5.2,1.1)],'Titanium',24)
 for j in range(3):
  a=j*math.tau/3;ob=box('Recessed fan blade',(0,0,0),(r*.73,1.75,.34),'Structure',.08);ob.location=(x+math.cos(a)*r*.4,y+math.sin(a)*r*.4,top-5.7);ob.rotation_euler.z=a+.14
...
for name,source in assets.items():
 ...
 for p in ob.data.polygons:
  ...
  for li in p.loop_indices:
   co=ob.data.vertices[ob.data.loops[li].vertex_index].co;uv.data[li].uv=(co[axes[0]]*.25 + (p.index % 17) * 2,co[axes[1]]*.25 + (p.index // 17) * 2)
...
data=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',data);studio.objects.link(cam);scene.camera=cam;data.type='ORTHO';data.ortho_scale=174
...
cam.location=(-155,-180,118);cam.rotation_euler=(Vector((0,0,43))-cam.location).to_track_quat('-Z','Y').to_euler();data.ortho_scale=138
