...
**New agents: read [CURRENT-GAME-HANDOFF.md](docs/CURRENT-GAME-HANDOFF.md) first.** It supersedes the historical instructions and scene choices below. The owner resumed implementation and approved road smoothing, rain, the tunnel/atmosphere correction and animated billboard variety. This is a playable review candidate; owner acceptance remains open.

- **Current candidate:** `UnityProject/Assets/Scenes/AnimatedBillboardsStage6.unity` — larger animated video ads in sparse architectural clusters, now with facade-connected cabinet supports, balanced light and distinct animation pacing (candidate 06). Haze, rain and the dry tunnel are preserved. Prior scenes are retained.
...
[Latest native billboard comparison](docs/current-game/billboard-finish-2026-09-17/comparison.jpg) · [Approved direction reference](docs/current-game/evidence/underground-descent-reference.png) · [Public repository](https://github.com/emrickk/vector-rush)

---

## Historical project README — retained for context, not current startup instructions
...
[Public GitHub repository](https://github.com/emrickk/vector-rush) · [Build brief](docs/GPT6-BUILD-PROMPT.md) · [Development history](docs/development-history.md) · [Ship art plan](docs/ship-art-plan.md)
...
The baseline and lighting experiments are complete as a resumable checkpoint. The accepted fog correction adds city depth; the clearer reflection-off road is restored after the reflection candidates failed to show a worthwhile benefit. Stage 1 remains incomplete against its production road-response target. The resumed [road-response comparison](docs/next-road-pass.md) is complete: one smoother-road candidate failed the useful visual-gain gate and the exact control is restored. The [independent verdict](docs/road-reviews/004-resumed-smoothness-native.md) and [next-technique feasibility report](docs/road-reviews/005-reflection-feasibility.md) explain why. The isolated SSR prototype is now implemented and rejected for insufficient demonstrated visual gain; its corrected native pass executes, but the nine views do not show useful reflections. [SSR02 native comparison](docs/ssr02-native-comparison.html) · [SSR verdict](docs/visual-target-reviews/001-reflection-feasibility.md). The original playable app remains unchanged; the source retains separate experimental infrastructure. See the [current handoff](docs/HANDOFF.md) and [independent A/B/C review](docs/environment-reviews/021-stage1-lighting-road-native.md). The earlier coastal-restoration suggestion is superseded by the chosen night direction.
