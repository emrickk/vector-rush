...
Five facade-connected service terraces integrate 30 kit prefab instances, supported shop roofs/equipment, wet surfaces and restrained practical lighting. Survey-invalid sites and unsupported roof props were corrected. Native review prompted a four-metre rise for the two right-side terraces so they read above the rail. All **2,169 original P4 Unity files** remain unchanged, including the animated billboards, road, handling, ship, HUD and weather. This is the bounded opening-city integration, not replacement of every tower or placement of every kit asset.

**290/290 EditMode tests pass.** Final native capture completes a 43.029-second lap with 780 source frames and recorded audio at 1920×1080. The foreground uncaptured three-lap race completes in 128.667 seconds with six finishers, zero recoveries and result/pause/restart checks passing. Delivered-frame P95/P99 are 9.025/9.317 ms, maximum 9.400 ms. The window changed from requested 1920×1080 to a logged 3326×2104 surface; do not label this a controlled 1080p benchmark. An earlier unfocused run failed the rival-finish timeout and is retained; no root cause or background-play fix is claimed.

App: `/Users/anping.wang/output/vector-rush-p4-city-integration-2026-09-17/NightCityStage7-02.app`. Native replay/comparisons: `http://127.0.0.1:8774/review-02/`; local file `review-02/index.html` under that output root. Actual native driving frames and replay were inspected, including the corrected storefront visibility and sheltered section. These checks use automated steering, not human/controller input. The owner approved committing and pushing this milestone on September 17, 2026. Owner artistic acceptance remains open. No speed-blur work is included. Next is owner review of this candidate; investigate the failed unfocused race separately if requested.

The prior execution-review blocker is resolved: the verified stale Unity/licensing processes were stopped with manual approval, and the installed Editor ran through the approved normal escalation path. No policy/security setting was changed. Older blocked/uncompiled statements below are historical.
