...
## Follow-up: build-time serialized changes

The integrator requested an additional read while the first build was running. This changes the earlier source-only finding: **a P1 retention defect is now evidenced by the generated pipeline settings.**

- **P1 — SSR OFF shader variants are stripped from a binary intended to compare runtime off/on.** `VectorPipeline.asset` changes `m_PrefilteringModeScreenSpaceReflection` from `0` to `2`. The installed `UniversalRenderPipelineAssetPrefiltering.cs` enum defines `0=Remove`, `1=Select`, `2=SelectOnly`; its SSR `SelectOnly` filter retains only the enabled keyword. `ShaderBuildPreprocessor.cs:1285` expressly selects it when `stripUnusedVariants && everyRendererHasSSR`, with a comment that the OFF variant is removed. Lit uses `#pragma multi_compile_fragment _ _SCREEN_SPACE_REFLECTION`. Registering an always-active feature while controlling its execution through a runtime volume does not tell this stripper to retain the disabled variant. Do not treat the first binary as a valid off/on control until corrected and rebuilt. No native visual corruption is asserted by this source finding.
- **Bounded corrective route:** retain a second, separately saved renderer asset matching the active renderer's Forward+/SSAO/settings, with SSR removed, and append it to the pipeline renderer list while preserving the original renderer and default index. `ShaderBuildPreprocessor.cs:748–776` evaluates every non-null list entry, combines their feature union, and clears `everyRendererHasSSR` when any entry lacks SSR. The resulting SSR prefilter should be `1=Select`, keeping both enabled and disabled states. The later `ShaderScriptableStripper.cs:1361–1392` removes a variant only when every collected renderer feature set rejects it. Matching the other renderer requirements avoids the broad expansion from disabling all unused-variant stripping. This is a source-supported recommendation, not yet an implemented/verified correction in this report snapshot.
- **Intended smoothness retention:** `m_PrefilterWriteSmoothness: 1 -> 0` is not disabling smoothness output. The field is assigned `stripWriteSmoothness`; `[ShaderKeywordFilter.RemoveIf(true, ...WriteSmoothness)]` removes the variant only when true. False retains the write-smoothness variants needed by SSR. Native data remains unverified.
- **Default volume addition:** `Assets/DefaultVolumeProfile.asset` gains one active SSR subasset and a component reference; `mode=1` means OpaquesOnly. The remaining added values are package defaults, several intentionally different from the preview runtime overrides. The bootstrap creates the explicit global profile before rendering, and `Configure` overrides mode and the planned experiment fields, so the default subasset does not defeat default-off semantics in this scene's normal path. It does mean this global default profile itself enables SSR for a camera/scene lacking that runtime override. Preserve this distinction and inspect the native effective volume rather than reporting serialized default values as the experiment configuration.
- **Renderer asset:** one active SSR subasset with `afterOpaque=0` is added to the existing feature list; the prior SSAO entry remains and its settings are unchanged.
- **Scene:** the observed `Solstice.unity` diff is only consistent regeneration of the bootstrap GameObject/Transform/component IDs and root reference. Script GUID, transform values and object settings are unchanged; no geometry/gameplay change is evidenced.

The first source report's reviewed file hashes remain its original snapshot. This follow-up reads generated asset diffs during the build; correction, final serialized verification and all native gates remain pending with the integrator.
