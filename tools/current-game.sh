...
case "${1:-help}" in
  open)
    ...
    ;;
  build|baseline-build|p4-build|stage7-build|koi-build|holo-koi-build)
    ...
    if [[ "${1}" == holo-koi-build ]]; then task_scene="Assets/Scenes/HolographicKoiStage10.unity"; fi
  ...
  test)
    ...
    ;;
  *) echo 'Usage: bash tools/current-game.sh open | build <fresh absolute .app> <fresh absolute evidence> | holo-koi-build <app> <evidence> | koi-build <app> <evidence> | stage7-build <app> <evidence> | p4-build <app> <evidence> | baseline-build <app> <evidence> | test [fresh absolute directory]' ;;
esac
